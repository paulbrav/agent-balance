...
def test_slow_burn_rides_to_high_threshold(cfg):
    ...
    ab.tick(
        cfg,
        now=NOW,
        fetcher=make_fetcher({"tok-alt1": usage(98, 40, H2, HD)}),
        out=out.append,
    )
...
def test_fresh_probes_append_history(cfg):
    ...
    def history():
        return [
            ...
            for e, f in (
                line.split()
                for line in (cfg.cache / "alt1.history").read_text().splitlines()
            )
        ]

    ab.probe(accts[0], cfg, NOW, make_fetcher({"tok-alt1": usage(42, 10, H2, HD)}))
