...
def setup_installed(cfg, account):
    ...
    ab.write_state(cfg, {"installed": account.name, "blob_sha256": sha,
...
def test_first_tick_bootstraps_and_installs_best(cfg):
    ...
    assert (cfg.pool / "projects").resolve() == \
        (cfg.root / "alt2" / "projects").resolve()
    assert json.loads((cfg.pool / ".claude.json").read_text())[
...
def test_installed_dir_deleted_hard_swaps(cfg):
    ...
    (cfg.pool / ".credentials.json").write_text(json.dumps({
        "claudeAiOauth": {"accessToken": "tok-alt3",
